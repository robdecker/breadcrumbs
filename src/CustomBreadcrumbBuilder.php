...
class CustomBreadcrumbBuilder implements BreadcrumbBuilderInterface {
  ...
  public function applies(RouteMatchInterface $route_match) {
    ...
    if (isset($parameters['node']) && $parameters['node']->getType() == 'my_cool_content') {
      return TRUE;
    }
  ...
  public function build(RouteMatchInterface $route_match) {
    ...
    foreach ($paths as $path) {
      $link = $this->getLinkForPath($path, $exclude, $breadcrumb);
      if (isset($link)) {
        $links[] = $link;
      }
    }
    ...
    if (isset($parameters['node'])) {
      // Add the plain-text title of the current node.
      $links[] = Link::createFromRoute($parameters['node']->getTitle(), '<none>');
    }
  ...
  /**
   * Generates a link from a path, if the path is in the router.
   *
   * @param string $path
   ...
   *   A HTML link or NULL if the path couldn't be matched.
   */
  protected function getLinkForPath($path, $exclude, &$breadcrumb) {
    ...
    if ($route_request) {
      ...
      if ($access->isAllowed()) {
        ...
        if (!isset($title)) {
          ...
          $title = str_replace(array('-', '_'), ' ', Unicode::ucfirst(end($path)));
        }
